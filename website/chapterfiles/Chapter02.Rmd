---
title: "IS5 in R: Displaying and Describing Data (Chapter 2)"
author: "Margaret Chien and Nicholas Horton (nhorton@amherst.edu)"
date: "June 10, 2018"
output: 
  pdf_document:
    fig_height: 4
    fig_width: 6
  html_document:
    fig_height: 3
    fig_width: 5
  word_document:
    fig_height: 4
    fig_width: 6
---


```{r, include = FALSE}
# Don't delete this chunk if you are using the mosaic package
# This loads the mosaic and dplyr packages
require(mosaic)
```

```{r, include = FALSE}
# knitr settings to control how R chunks work.
require(knitr)
opts_chunk$set(
  tidy = FALSE,     # display code as typed
  size = "small"    # slightly smaller font for code
)
```

## Introduction and background 

This document is intended to help describe how to undertake analyses introduced 
as examples in the Fifth Edition of \emph{Intro Stats} (2018) by De Veaux, Velleman, and Bock.
More information about the book can be found at http://wps.aw.com/aw_deveaux_stats_series.  This
file as well as the associated R Markdown reproducible analysis source file used to create it can be found at http://nhorton.people.amherst.edu/is5.

This work leverages initiatives undertaken by Project MOSAIC (http://www.mosaic-web.org), an NSF-funded effort to improve the teaching of statistics, calculus, science and computing in the undergraduate curriculum. In particular, we utilize the `mosaic` package, which was written to simplify the use of R for introductory statistics courses. A short summary of the R needed to teach introductory statistics can be found in the mosaic package vignettes (http://cran.r-project.org/web/packages/mosaic).  A paper describing the mosaic approach was published in the *R Journal*: https://journal.r-project.org/archive/2017/RJ-2017-024.

## Chapter 2: Displaying and Describing Data

### Section 2.1: Summarizing and Displaying a Categorical Variable

See displays on page 19-23.

```{r message = FALSE}
library(mosaic)
library(readr)
options(digits = 3)
Titanic <- read_csv("http://nhorton.people.amherst.edu/is5/data/Titanic.csv")
tally(~ Class, data = Titanic)
tally(~ Class, format = "percent", data = Titanic)
barchart(tally(~ Class, data = Titanic))
```

### Section 2.2: Displaying a Quantitative Variable

#### Ages of Those Aboard the the Titanic

```{r}
gf_histogram(~ Age, data = Titanic, bins = 15, ylab = "# of People")
```

#### Earthquakes and Tsunamis

```{r}
Earthquakes <- read_csv("http://nhorton.people.amherst.edu/is5/data/Tsunamis_2016.csv")
gf_histogram(~Primary_Magnitude, data = Earthquakes, bins = 12, ylab = "Earthquakes", xlab = "Magnitude")
```

#### Stem-and-Leaf Displays

See examples in textbook.

```{r}
#Don't think there is data available for this
```

#### Dotplot

```{r}
Derby <- read_csv("http://nhorton.people.amherst.edu/is5/data/Kentucky_Derby_2016.csv")
gf_dotplot(~ Time_Sec, data = Derby, binwidth = 1)
```

#### Density Plots

```{r}
gf_dens(~ Age, data = Titanic, ylab = "Density")
```

### Section 2.3: Shape

See displays on pages 28-29.

#### Consumer Price Index

```{r}
CPI <- read_csv("http://nhorton.people.amherst.edu/is5/data/CPI_Worldwide.csv")
gf_histogram(~ `Consumer.Price.Index`, data = CPI, ylab = "# of Cities", xlab = "Consumer Price Index")
```

#### Credit Card Expenditures

```{r}
CreditCardEx <- read_csv("http://nhorton.people.amherst.edu/is5/data/Credit_card_charges.csv")
CreditCardEx <- CreditCardEx %>%
  rename(Charges = 'Charges($)') #The $ was giving me other errors so I renamed it
gf_histogram(~ Charges, data = CreditCardEx, ylab = "# of Customers", xlab = "Average Monthly Expenditure ($)")
```

### Section 2.4: Center

#### Finding Median and Mean

```{r}
gf_histogram(~ Age, data = filter(Titanic, Class == "Crew"))
favstats(~ Age, data = filter(Titanic, Class == "Crew"))
```

### Section 2.5: Spread

#### IQR

```{r}
favstats(~ Age, data = filter(Titanic, Class == "Crew"))
```

XX MC do we want to pipe here?



We can find IQR by subtracting Q1 from Q3.  

### Summarizing a Distribution

XX MC please be sure that code doesn't flow into right margin...

```{r}
Nissan <- read_csv("http://nhorton.people.amherst.edu/is5/data/Nissan.csv")
gf_histogram(~ mpg, data = Nissan, bins = 18, xlab = "Fuel Efficiency (mpg)", ylab = "# of Fill-ups")
favstats(~ mpg, data = Nissan)
```

### Random Matters

```{r}
Commute <- read_csv("http://nhorton.people.amherst.edu/is5/data/Population_Commute_Times.csv")
gf_histogram(~ `Commute.Time`, data = Commute, binwidth = 10, xlab = "Commute Time (min)", ylab = "# of Employees")
```
