...
```{r}
# Figure 9.6
tempplm <- lm(pct_bf ~ height, data = BodyFat) # or do i use remef?
# gf_point(resid(housinglm) ~ )
```
...
gf_point(MSRP ~ (Displacement)^(1/3), color = ~ Cooling, data = DirtBikes) %>%
  gf_lm()
bikeslm <- lm(MSRP ~ Displacement^(1/3) + Cooling, data = DirtBikes) # not sure why there's an error
# msummary(bikeslm)
```
