...
The `I()` function keeps the class of an object the same. Here we use it to keep the variable `Displacement` "as is" to prevent an error.  
...
tempdata <- augment(diamondlm) %>%
  clean_names()
str(tempdata)
gf_point(sqrt_price ~ carat_size, color = ~ color, data = tempdata) %>%
  gf_lm(sqrt_price ~ carat_size, color = ~ color) %>%
