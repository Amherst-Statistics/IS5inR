...
CIdata <- CIdata %>%
  mutate(lowerbound = mean - 1.96 * sd/(100^.5), upperbound = mean + 1.96 * sd/(100^.5)) # use t test function
CIdata %>%
  select(lowerbound, upperbound) # make function that returns center, lower, upper, t test, recreating process is a bit harder, but here (line plot, point plot in the middle)
