...
MenWeight <- read_csv("http://nhorton.people.amherst.edu/is5/data/Mens_Weights.csv") %>%
  clean_names()
gf_histogram(~ weight_in_kg, data = MenWeight, xlab = "Weight (kg)", ylab = "# of Men",
             binwidth = 10)
...
gf_histogram(~ (weight_in_kg - 74), data = MenWeight, xlab = "Kg Above Recommended Weight", 
             ylab = "# of Men", binwidth = 10)
```
...
Commute <- read_csv("http://nhorton.people.amherst.edu/is5/data/Population_Commute_Times.csv") %>%
  clean_names()
gf_histogram(~ commute_time, data = Commute, xlab = "Commute Times (min)", ylab = "# of Employees", 
             binwidth = 10)
set.seed(2143)
Commute_sample <- do(10000) * mean(sample(Commute$commute_time, 100)) 
