...
XX Some reference as to where we are? pg. 123.
```{r}
...
XX Why are we piping through `data.frame()` Filter should already return a tibble
```{r}
...
filter(WomenHeptathlon2016, last_name == "Johnson-Thompson") %>%
  data.frame()
```
XX If we delete the `data.frame()` call then we would also delete this.
...
XX What exactly is happening here? Can a sentence be included on what we are trying to show?
...
# What does do() do? XX Does this comment have to be here?
