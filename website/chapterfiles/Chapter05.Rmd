...
gf_histogram(~ commute_time, data = Commute, xlab = "Commute Times (min)", ylab = "# of Employees", 
             binwidth = 10, center = 5)
set.seed(2143) # For reproducibility
numsim <- 10000 # Number of simulations

# What does do() do?

mean(~ commute_time, data = sample(Commute, 100)) # Mean of one random sample
mean(~ commute_time, data = sample(Commute, 100)) # Mean of another random sample

do(2) * mean(~ commute_time, data = sample(Commute, 100)) # Carries out mean() twice

# For the visualization, we use do() 10,000 times
Commute_sample <- do(numsim) * mean(~ commute_time, data = sample(Commute, 100)) 
```
