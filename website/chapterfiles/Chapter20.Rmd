...
The histogram is not as normal as it is in the book because we only did 25 trials instead of 1,000.  
...
msummary(coasterlm) 
coastermlm <- lm(Duration ~ Drop + Speed, data = Coasters) # multiple linear model
msummary(coastermlm)
```
