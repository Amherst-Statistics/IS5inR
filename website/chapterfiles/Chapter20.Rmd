...
Here we fit `price` by `carat_size`, which differs somewhat from the display in the book.  

```{r}
diamondlm <- lm(price ~ carat_size, data = Diamonds) 
...
Coasters <- Coasters %>%
  filter(Name != "Tower of Terror" | Name != "Wild Beast") %>%
  mutate(Inversions = as.factor(Inversions))
coasterlm <- lm(Duration ~ Drop, data = Coasters)
# Numbers are a bit off because the book has removed some rollercoasters
msummary(coasterlm) 
coastermlm <- lm(Duration ~ Drop + Speed, data = Coasters)
msummary(coastermlm)

# Temporary code that isn't matching
Coasterstemp <- Coasters %>%
  filter(Drop != "NA") %>%
  filter(Duration != "NA") %>%
  filter(Name != "Tower of Terror") %>%
  filter(Name != "Wild Beast")
coastertemp <- lm(Duration ~ Drop, data = Coasterstemp)
msummary(coastertemp)
coastermlmtemp <- lm(Duration ~ Drop + Speed, data = Coasterstemp)
msummary(coastermlmtemp)
```
