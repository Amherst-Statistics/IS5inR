...
gf_dist("norm", mean = 0, sd = 1, fill = ~ cut(x, c(-Inf, 2, 100, Inf)), geom = "area", 
        alpha = .5) %>%
  gf_dist("norm", mean = 4, sd = 1, fill = ~ cut(x, c(-Inf, -100, 2, Inf)), geom = "area", 
          alpha = .5) %>%
  gf_labs(x = "p", y = "") %>% 
  ...
  guides(fill = FALSE) # Improve on the colors?
```
