...
```{r}
barplot(tally(~ cats_dogs_both + gender, data = OKCupid), beside = TRUE)
#There are many who don't own either
barplot(tally(~ cats_dogs_both + gender, data = filter(OKCupid, cats_dogs_both != "NA")), beside = TRUE)
```
...
Berk_2 <- rbind(
  ...
  do(318) * data.frame(admit = FALSE, sex = "F", school = "F") # some numbers are off but it's really annoying to fix
)
...
school <- c(rep("A", 512), rep("B", 313), rep("C", 120), rep("D", 138), rep("E", 53), rep("F", 22),
            rep("A", 512/.621 - 512), rep("B", 313/.602 - 313), rep("C", 120/.369 - 120), rep("D", 138/.331 - 138), rep("E", 53/.277 - 53), rep("F", 22/.059 - 22),
            rep("A", 89), rep("B", 17), rep("C", 202), rep("D", 131), rep("E", 94), rep("F", 24), 
            rep("A", 89/.824 - 89 + 1), rep("B", 17/.68 - 17 + 1), rep("C", 202/.341 - 202 + 1), rep("D", 131/.349 - 131 + 1), rep("E", 94/.239 - 94 + 1), rep("F", 24/.07 - 24)) # + 1 because the rows weren't matching up
