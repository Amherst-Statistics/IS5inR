...
gf_histogram(~ resid(cereallm), bins = 12) %>%
  gf_labs(x = "Residuals (calories)", y = "# of Cereals")
gf_point(jitter(resid(cereallm)) ~ jitter(fitted(cereallm)))
```
...
gf_point(jitter(calories) ~ sugars, color = ~ shelfgroup, data = Cereals) %>%
  gf_lm() %>%
...
gf_point(Men ~ Year, data = MarriageAge, ylab = "Men's Age at First Marriage")
```
...
Election2000 %>%
  filter(Buchanan <= 3000) %>%
  gf_point(Buchanan ~ Nader) %>%
  gf_lm()
...
gf_histogram(~ resid(handlm), bins = 7) %>%
...
gf_histogram(~ resid(penguinlm), bins = 9) %>%
  gf_labs(x = "Residuals", y = "")
```
...
gf_point(resid(penguinlm2) ~ fitted (penguinlm2)) %>%
  gf_labs(x = "Predicted Values", y = "Residuals")
gf_histogram(~ resid(penguinlm2), bins = 10, xlab = "Residuals", ylab = "# of Residuals")
```
...
gf_point(resid(penguinlm3) ~ fitted(penguinlm3)) %>%
  gf_labs(x = "Predicted Values", y = "Residuals")
gf_histogram(~ resid(penguinlm3), bins = 10, xlab = "Residuals", ylab = "# of Residuals")
```
