...
```{r}
gf_point(dive_heart_rate ~ duration_min, data = Penguins, xlab = "Duration (min)", 
         ylab = "Dive Heart Rate (bpm)")
penguinlm <- lm(dive_heart_rate ~ duration_min, data = Penguins)
gf_point(resid(penguinlm) ~ duration_min, data = Penguins, xlab = "Duration (min)", 
         ylab = "Residuals")
```
...
Cereal <- Cereal %>%
  ...
  mutate(resid_cat = ifelse(residuals >= -30 & residuals <= 25, "mid", 
                            ifelse(residuals > 25, "high", "low"))) # For color categories
# Figure 8.4
...
gf_point(resid(penguinlm) ~ fitted(penguinlm), data = Penguins, xlab = "Predicted Values", 
         ylab = "Residuals")
