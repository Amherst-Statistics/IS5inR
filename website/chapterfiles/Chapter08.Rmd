...
# Figure 8.3, page 235
gf_histogram(~ resid(cereallm), bins = 12, center = 0) %>%
  gf_labs(x = "Residuals (calories)", y = "# of Cereals")

Cereal <- Cereal %>%
  mutate(residuals = resid(cereallm),
         fitted = fitted(cereallm)) %>%
  mutate(resid_cat = ifelse(residuals >= -30 & residuals <= 25, "mid", ifelse(residuals > 25, "high", "low"))) # there must be a better way to do this!
# Figure 8.4
