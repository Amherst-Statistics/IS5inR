...
# Figure 14.4
set.seed(14354)
numsim <- 1000 ## XX MC keep original numsim?  Why redefine?
...
The `confint()` requires function takes as argument a linear model object.  
...
set.seed(134)
numsim <- 10000
# XX MC what other resample functions are we loading?
commutebootstrap <- do(numsim) * mean(~ time, data = mosaic::resample(CommuteSample))
...
confint(cbootlm)
t.test(commutebootstrap$mean, conf.level = 0.95)  # XX MC why $?
```
