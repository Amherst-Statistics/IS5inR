...
gf_point(mom_age ~ dad_age, data = sample(LiveBirths, 50))
# Graph will look different for different samples
cor(mom_age ~ dad_age, data = LiveBirths)

# What does do() do?
cor(mom_age ~ dad_age, data = sample(LiveBirths, 50)) # Correlation of one random sample
cor(mom_age ~ dad_age, data = sample(LiveBirths, 50)) # Correlation of another random sample

do(2) * cor(mom_age ~ dad_age, data = sample(LiveBirths, 50)) # Finds the correlation twice

# For the visualization, we need 10,000 correlations
LiveCorr <- do(numsim) * cor(mom_age ~ dad_age, data = sample(LiveBirths, 50))
```

The `do()` function runs, 10,000 times, the correlation and sampling functions on a random sample of 50.  

```{r}
gf_histogram(~ LiveCorr, xlab = "Correlation of Mother's Age and Father's Age in Samples of Size 50", 
             ylab = "# of Samples")
```
...
gf_point(log(f_stop) ~ shutter_speed, data = FStops, xlab = "Shutter Speed (sec)", ylab = "Log (f/stop)")
# Figure 6.12
gf_point((f_stop)^2 ~ shutter_speed, data = FStops, xlab = "Shutter Speed (sec)", ylab = "f/stop squared")
```
