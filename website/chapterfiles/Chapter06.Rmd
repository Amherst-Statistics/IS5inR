...
LiveBirths <- read_csv("http://nhorton.people.amherst.edu/is5/data/Babysamp_98.csv") %>%
  clean_names()
set.seed(14513)
gf_point(mom_age ~ dad_age, data = sample(filter(LiveBirths, dad_age != "NA"), 50))
# Graph will look different for different samples
cor(mom_age ~ dad_age, data = filter(LiveBirths, dad_age != "NA"))
LiveCorr <- do(10000) * cor(mom_age ~ dad_age, data = sample(filter(LiveBirths, dad_age != "NA"), 50))
gf_histogram(~ LiveCorr, xlab = "Correlation of Mother's Age and Father's Age in Samples of Size 50", ylab = "# of Samples")
```
